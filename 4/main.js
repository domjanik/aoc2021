...
function checkRows(cards) {
    let unmarkedCards;
    let won = false;
    for(let row = 0; row < 5; row++) {
        const markedCards = cards.filter(card => card.x === row && card.marked);
        if(markedCards.length === 5) {
            won = true;
            unmarkedCards = cards.filter(card => !card.marked);
            break;
...
function checkIfWon(cards, checkedNumber) {
    let winningBoardUnmarkedNumbers;
    let marked = cards.filter(card => card.marked == true);
    if(marked.length >= 5) {
        const rowResult = checkRows(cards);
        if(rowResult.won) {
            winningBoardUnmarkedNumbers = rowResult.unmarkedCards;
        }
        const winningColumn = checkColumns(cards);
        if(winningColumn.won) {
            winningBoardUnmarkedNumbers = winningColumn.unmarkedCards;
        }
    }

    if(winningBoardUnmarkedNumbers && winningBoardUnmarkedNumbers.length > 0 ) {
        return {
            isWinner: true,
            score: winningBoardUnmarkedNumbers.map(el=>el.value).reduce((a,b)=>a+b) * checkedNumber
        };
    } else {
        return {
            isWinner: false
...
function prepareBoards() { 
    ...
    for(let i = 0; i < boards.length; i++) {
        ...
        preparedBoards.push(preparedBoard);
    }
    return preparedBoards;
}

function findWinners() {
    const preparedBoards = prepareBoards();

    for(const number of numbers){
        for (const board of preparedBoards) {
            if (board.hasWon) {
                board.numbers
                    .filter(numberOnBoard=>numberOnBoard.value==number)
                    .forEach(numberOnBoard => numberOnBoard.marked = true);
                
                let result = checkIfWon(board.numbers, number);
                if(result.isWinner) {
                    board.hasWon = false;
                    winners.push(result.score);
                }  
            }
        }
    }
...
console.log(`
First winner score = ${firstWinner} [ ${firstWinner === 10680 ? "TRUE" : "FALSE"}  ]
Last winner score = ${lastWinner} [ ${lastWinner == 31892 ? "TRUE" : "FALSE"}  ]
`)